...
#include <gendoc/cppparser/CppTokenGrammar.hpp>
...
CppToken* cppTokenGrammar = nullptr;

HtmlSourceCodeGenerator::HtmlSourceCodeGenerator(const std::vector<std::u32string>& inputLines_, int numDigits_,
    const std::string& styleDirName_, const std::string& styleFileName_, SymbolTable& symbolTable_, int inlineCodeLimit_,
    std::unordered_map<std::u32string, std::unique_ptr<dom::Element>>& inlineCodeMap_) :
    symbolTable(symbolTable_), currentFunction(nullptr), invokeSymbol(nullptr), inputLines(inputLines_), numDigits(numDigits_), 
    currentSourceLineNumber(1), styleFilePath(Path::Combine(Path::Combine("../../..", styleDirName_), styleFileName_)), contentFilePath("../.."),
...
{
    htmlElement.reset(new dom::Element(U"html"));
}
...
void HtmlSourceCodeGenerator::WriteDocument()
{
    ...
    htmlSourceFileDocument.AppendChild(std::unique_ptr<dom::Node>(htmlElement.release()));
...
void HtmlSourceCodeGenerator::Visit(SourceFileNode& sourceFileNode)
{
    ...
    titleElement->AppendChild(std::unique_ptr<dom::Node>(new dom::Text(sourceFileNode.ProjectName() + U"/" + ToUtf32(sourceFileNode.RelativeSourceFilePath()))));
    ...
    bodyElement.reset(new dom::Element(U"body"));
...
std::u32string HtmlSourceCodeGenerator::MakeSymbolRef(Symbol* symbol)
{
    ...
    if (symbol->IsParentSymbol())
    {
        ...
        {
            return ToUtf32(Path::Combine(contentFilePath, ToUtf8(symbol->Id()) + ".html"));
        }
    }
...
void HtmlSourceCodeGenerator::WriteType(TypeSymbol* type, const std::vector<IdentifierNode*>& idNodeSequence)
{
        ...
        for (Derivation derivation : derivedTypeSymbol->Derivations())
        {
            switch (derivation)
            {
                ...
                case Derivation::base:
                {
                    ...
                    WriteType(baseType, idNodeSequence);
                    break;
                }
            }
        }
    ...
    else if (type->IsClassTemplateSpecializationSymbol())
    {
        ...
        WriteType(primaryType, symbolTable.GetIdNodeSequence(templateIdNode));
        writer.WriteOther(U"<");
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            WriteType(templateArgumentSymbol, symbolTable.GetIdNodeSequence(templateArgumentNode));
        }
        writer.WriteOther(U">");
    }
    else if (type->IsElaborateClassTypeSymbol())
    {
        ...
        writer.WriteSpace(1);
        WriteType(elaborateClassTypeSymbol->GetClassType(), idNodeSequence);
    }
    else
    {
        for (int i = 0; i < n - 1; ++i)
...
{
    inlineLineElement.reset(new dom::Element(U"span"));
...
void HtmlSourceCodeGenerator::CloseLine()
{
    ...
    if (inlineCodeElement && inlineLineElement)
    {
        inlineCodeElement->AppendChild(std::unique_ptr<dom::Node>(inlineLineElement.release()));
        inlineCodeElement->AppendChild(std::unique_ptr<dom::Node>(new dom::Element(U"br")));
...
void HtmlSourceCodeGenerator::UseInputLine()
{
    if (!cppTokenGrammar)
    {
        cppTokenGrammar = CppToken::Create();
    }
    const std::u32string& line = inputLines[currentSourceLineNumber - 1];
    OpenLine();
    cppTokenGrammar->Parse(line.c_str(), line.c_str() + line.length(), 0, "", &writer);
    CloseLine();
}
...
void HtmlSourceCodeGenerator::Visit(SpecialMemberFunctionNode& specialMemberFunctionNode)
{
    ...
    {
        int firstLineNumber = specialMemberFunctionNode.CtorInitializer()->GetSpan().LineNumber();
        int lastLineNumber = specialMemberFunctionNode.FunctionBody()->EndBraceSpan().LineNumber();
        if (lastLineNumber - firstLineNumber + 1 <= inlineCodeLimit)
        {
            inlineCodeElement.reset(new dom::Element(U"span"));
    ...
    if (specialMemberFunctionNode.FunctionBody())
    {
        if (inlineCodeElement)
        {
            inlineCodeOffset = specialMemberFunctionNode.FunctionBody()->GetSpan().Start() - (*lineStarts)[specialMemberFunctionNode.FunctionBody()->GetSpan().LineNumber() - 1];
        }
...
void HtmlSourceCodeGenerator::Visit(CompoundStatementNode& compoundStatementNode)
{
    if (blockLevel == 0)
    {
        int firstLineNumber = compoundStatementNode.GetSpan().LineNumber();
        int lastLineNumber = compoundStatementNode.EndBraceSpan().LineNumber();
        if (lastLineNumber - firstLineNumber + 1 <= inlineCodeLimit)
        {
            inlineCodeElement.reset(new dom::Element(U"span"));
            inlineCodeElement->SetAttribute(U"class", U"code");
...
void HtmlSourceCodeGenerator::Visit(TypeExprNode& typeExprNode)
{
    ...
    if (symbol->IsTypeSymbol())
    {
        ...
        WriteType(typeSymbol, idNodeSequence);
    }
    else
    {
...
void HtmlSourceCodeGenerator::Visit(SimpleTypeCastExpressionNode& simpleTypeCastExpressionNode)
{
    MoveTo(simpleTypeCastExpressionNode.GetSpan());
    simpleTypeCastExpressionNode.SimpleType()->Accept(*this);
    writer.WriteOther(U"(");
    idSequence.clear();
    firstInitializer = true;
    simpleTypeCastExpressionNode.Expr()->Accept(*this);
    WriteIdSequence(false, symbolTable.GetSymbolNothrow(simpleTypeCastExpressionNode.Expr()));
    writer.WriteOther(U")");
}
