...
#pragma warning(default: 4244)
    ...
    hppFormatter.WriteLine("#include <gendoc/parsing/Grammar.hpp>");
    hppFormatter.WriteLine("#include <gendoc/parsing/Keyword.hpp>");
    if (hppFileName != "StdLib.hpp")
    {
        cppFormatter.WriteLine("#include \"" + hppFileName + "\"");
    }
    cppFormatter.WriteLine("#include <gendoc/parsing/Action.hpp>");
    cppFormatter.WriteLine("#include <gendoc/parsing/Rule.hpp>");
    cppFormatter.WriteLine("#include <gendoc/parsing/ParsingDomain.hpp>");
    cppFormatter.WriteLine("#include <gendoc/parsing/Primitive.hpp>");
    cppFormatter.WriteLine("#include <gendoc/parsing/Composite.hpp>");
    cppFormatter.WriteLine("#include <gendoc/parsing/Nonterminal.hpp>");
    cppFormatter.WriteLine("#include <gendoc/parsing/Exception.hpp>");
    cppFormatter.WriteLine("#include <gendoc/parsing/StdLib.hpp>");
    cppFormatter.WriteLine("#include <gendoc/parsing/XmlLog.hpp>");
    cppFormatter.WriteLine("#include <gendoc/util/Unicode.hpp>");
    ...
    for (NamespaceContentMapIt i = nsContent.begin(); i != e; ++i)
    {
        ...
        if (!content.UsingObjects().empty())
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                gendoc::codedom::UsingObject* subject = usingObject->Subject();
        ...
        cppFormatter.WriteLine("using namespace gendoc::parsing;");
        cppFormatter.WriteLine("using namespace gendoc::util;");
        cppFormatter.WriteLine("using namespace gendoc::unicode;");
        cppFormatter.WriteLine();
