...
#include <gendoc/cppsym/IntegerLiteralTypeSymbol.hpp>
...
{
    std::unique_ptr<dom::Document> symbolTableDoc(new dom::Document());
    symbolTableDoc->AppendChild(std::move(ToDomElement()));
    return symbolTableDoc;
}

std::unique_ptr<dom::Element> SymbolTable::ToDomElement()
{
    std::unique_ptr<dom::Element> symbolTableElement(new dom::Element(U"symbolTable"));
    std::unique_ptr<dom::Element> globalNsElement = globalNs.ToDomElement();
    symbolTableElement->AppendChild(std::unique_ptr<dom::Node>(globalNsElement.release()));
    std::unique_ptr<dom::Element> typesElement = CreateTypesElement();
    if (typesElement)
    {
...
std::unique_ptr<dom::Element> SymbolTable::CreateTypesElement()
{
    std::unique_ptr<dom::Element> typesElement;
    if (!types.empty())
    {
        typesElement.reset(new dom::Element(U"types"));
        ...
        {
            std::unique_ptr<dom::Element> typeElement = typeSymbol->ToDomElement();
