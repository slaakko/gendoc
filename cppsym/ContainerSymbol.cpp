...
std::unique_ptr<dom::Element> ContainerSymbol::Namespaces()
{
    std::unique_ptr<dom::Element> namespacesElement;
    ...
    if (!namespaces.empty())
    {
        namespacesElement.reset(new dom::Element(U"namespaces"));
        std::sort(namespaces.begin(), namespaces.end(), ByName());
        for (NamespaceSymbol* ns : namespaces)
        {
            std::unique_ptr<dom::Element> namespaceElement = ns->ToDomElement();
            namespacesElement->AppendChild(std::unique_ptr<dom::Node>(namespaceElement.release()));
        }
    }
    return namespacesElement;
}

std::unique_ptr<dom::Element> ContainerSymbol::Classes()
{
    std::unique_ptr<dom::Element> classesElement;
    ...
    if (!classes.empty())
    {
        classesElement.reset(new dom::Element(U"classes"));
        std::sort(classes.begin(), classes.end(), ByName());
        for (ClassTypeSymbol* cls : classes)
        {
            std::unique_ptr<dom::Element> classElement = cls->ToDomElement();
            classesElement->AppendChild(std::unique_ptr<dom::Node>(classElement.release()));
        }
    }
    return classesElement;
}

std::unique_ptr<dom::Element> ContainerSymbol::Constructors()
{
    std::unique_ptr<dom::Element> constructorsElement;
    ...
    {
        constructorsElement.reset(new dom::Element(U"constructors"));
        std::sort(constructors.begin(), constructors.end(), ByName());
        for (ConstructorSymbol* constructor : constructors)
        {
            std::unique_ptr<dom::Element> constructorElement = constructor->ToDomElement();
            constructorsElement->AppendChild(std::unique_ptr<dom::Node>(constructorElement.release()));
        }
    }
    return constructorsElement;
}

std::unique_ptr<dom::Element> ContainerSymbol::Functions()
{
    std::unique_ptr<dom::Element> functionsElement;
    ...
    if (!functions.empty())
    {
        functionsElement.reset(new dom::Element(U"functions"));
        std::sort(functions.begin(), functions.end(), ByName());
        for (FunctionSymbol* function : functions)
        {
            std::unique_ptr<dom::Element> functionElement = function->ToDomElement();
            functionsElement->AppendChild(std::unique_ptr<dom::Node>(functionElement.release()));
        }
    }
    return functionsElement;
}

std::unique_ptr<dom::Element> ContainerSymbol::EnumTypes()
{
    std::unique_ptr<dom::Element> enumTypesElement;
    ...
    if (!enumTypes.empty())
    {
        enumTypesElement.reset(new dom::Element(U"enumTypes"));
        std::sort(enumTypes.begin(), enumTypes.end(), ByName());
        for (EnumTypeSymbol* enumType : enumTypes)
        {
            std::unique_ptr<dom::Element> enumTypeElement = enumType->ToDomElement();
            enumTypesElement->AppendChild(std::unique_ptr<dom::Node>(enumTypeElement.release()));
        }
    }
    return enumTypesElement;
}

std::unique_ptr<dom::Element> ContainerSymbol::Enumerators()
{
    std::unique_ptr<dom::Element> enumeratorsElement;
    ...
    {
        enumeratorsElement.reset(new dom::Element(U"enumerators"));
        for (EnumeratorSymbol* enumerator : enumerators)
        {
            std::unique_ptr<dom::Element> enumeratorElement = enumerator->ToDomElement();
            enumeratorsElement->AppendChild(std::unique_ptr<dom::Node>(enumeratorElement.release()));
        }
    }
    return enumeratorsElement;
}

std::unique_ptr<dom::Element> ContainerSymbol::Typedefs()
{
    std::unique_ptr<dom::Element> typedefsElement;
    ...
    if (!typedefs.empty())
    {
        typedefsElement.reset(new dom::Element(U"typedefs"));
        std::sort(typedefs.begin(), typedefs.end(), ByName());
        for (TypedefSymbol* typedef_: typedefs)
        {
            std::unique_ptr<dom::Element> typedefElement = typedef_->ToDomElement();
            typedefsElement->AppendChild(std::unique_ptr<dom::Node>(typedefElement.release()));
        }
    }
    return typedefsElement;
}

std::unique_ptr<dom::Element> ContainerSymbol::Variables()
{
    std::unique_ptr<dom::Element> variablesElement;
    ...
    if (!variables.empty())
    {
        variablesElement.reset(new dom::Element(U"variables"));
        std::sort(variables.begin(), variables.end(), ByName());
        for (VariableSymbol* variable : variables)
        {
            std::unique_ptr<dom::Element> variableElement = variable->ToDomElement();
            variablesElement->AppendChild(std::unique_ptr<dom::Node>(variableElement.release()));
        }
    }
    return variablesElement;
}

std::unique_ptr<dom::Element> ContainerSymbol::Grammars()
{
    std::unique_ptr<dom::Element> grammarsElement;
    ...
    if (!grammars.empty())
    {
        grammarsElement.reset(new dom::Element(U"grammars"));
        std::sort(grammars.begin(), grammars.end(), ByName());
        for (GrammarSymbol* grammar : grammars)
        {
            std::unique_ptr<dom::Element> grammarElement = grammar->ToDomElement();
