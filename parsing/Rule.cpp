...
using gendoc::codedom::IdExpr;

class CodeExpandingVisitor : public gendoc::codedom::Visitor
{
public:
    ...
    ParsingObject(name_, grammar_->GetScope(), ObjectKind::ruleLink), linkedRuleName(linkedRuleName_), grammar(grammar_)
{
    ...
    if (prefix == U"stdlib")
    {
        std::u32string projectPrefix = grammar->GetParsingDomain()->ProjectPrefix();
        linkedRuleName = projectPrefix + U".parsing.stdlib." + GetCommonName(linkedRuleName);
    }
}
