...
TypeResolver::TypeResolver(SymbolTable& symbolTable_, ContainerScope* containerScope_, BoundSourceFile& boundSourceFile_, TypeResolverFlags flags_) :
    ...
    type(nullptr), classGroup(nullptr), classKey(ClassKey::none), parentResolved(true)
{
}
...
void TypeResolver::Visit(TemplateIdNode& templateIdNode)
{
    ...
    {
        type = symbolTable.MakeClassTemplateSpecializationSymbol(span, primaryClassTemplate, &templateIdNode, templateArgumentTypes, templateArgumentNodes);
    }
}

void TypeResolver::Visit(TemplateArgumentNode& templateArgumentNode)
{
    templateArgumentTypes.push_back(ResolveType(symbolTable, containerScope, boundSourceFile, templateArgumentNode.Arg()));
