...
void StatementBinder::Visit(SpecialMemberFunctionNode& specialMemberFunctionNode)
{
    ...
    if (symbol)
    {
        if (symbol->IsFunctionSymbol())
        {
            ...
            {
                BindExpression(specialMemberFunctionNode.CtorInitializer(), symbolTable, containerScope, currentSourceFile, currentFunction);
            }
...
{
    BindExpression(ifStatementNode.Condition(), symbolTable, containerScope, currentSourceFile, currentFunction);
...
{
    BindExpression(switchStatementNode.Condition(), symbolTable, containerScope, currentSourceFile, currentFunction);
    switchStatementNode.Statement()->Accept(*this);
}

void StatementBinder::Visit(CaseStatementNode& caseStatementNode)
{
    BindExpression(caseStatementNode.CaseExpr(), symbolTable, containerScope, currentSourceFile, currentFunction);
...
void StatementBinder::Visit(ExpressionStatementNode& expressionStatementNode)
{
    ...
    {
        BindExpression(expressionStatementNode.Child(), symbolTable, containerScope, currentSourceFile, currentFunction);
    }
}

void StatementBinder::Visit(WhileStatementNode& whileStatementNode)
{
    BindExpression(whileStatementNode.Condition(), symbolTable, containerScope, currentSourceFile, currentFunction);
...
void StatementBinder::Visit(DoStatementNode& doStatementNode)
{
    ...
    BindExpression(doStatementNode.Condition(), symbolTable, containerScope, currentSourceFile, currentFunction);
}
...
void StatementBinder::Visit(RangeForStatementNode& rangeForStatementNode)
{
    ...
    if (symbol->IsDeclarationBlockSymbol())
    {
        ...
        ContainerScope* prevContainerScope = containerScope;
        containerScope = declarationBlock->GetContainerScope();
        BindExpression(rangeForStatementNode.ForRangeDeclaration(), symbolTable, containerScope, currentSourceFile, currentFunction);
        BindExpression(rangeForStatementNode.ForRangeInitializer(), symbolTable, containerScope, currentSourceFile, currentFunction);
...
void StatementBinder::Visit(ForStatementNode& forStatementNode)
{
    ...
    if (symbol->IsDeclarationBlockSymbol())
    {
        ...
        {
            BindExpression(forStatementNode.Condition(), symbolTable, containerScope, currentSourceFile, currentFunction);
        }
        if (forStatementNode.LoopExpr())
        {
            BindExpression(forStatementNode.LoopExpr(), symbolTable, containerScope, currentSourceFile, currentFunction);
        }
...
void StatementBinder::Visit(ReturnStatementNode& returnStatementNode)
{
    ...
    {
        BindExpression(returnStatementNode.ReturnExpr(), symbolTable, containerScope, currentSourceFile, currentFunction);
    }
}

void StatementBinder::Visit(DeclarationStatementNode& declarationStatementNode)
{
    BindExpression(declarationStatementNode.Child(), symbolTable, containerScope, currentSourceFile, currentFunction);
}
...
{
    BindExpression(handlerNode.ExceptionDeclaration(), symbolTable, containerScope, currentSourceFile, currentFunction);
