...
TypeBinder::TypeBinder(SymbolTable& symbolTable_) : 
    symbolTable(symbolTable_), containerScope(nullptr), currentSourceFile(nullptr), currentClass(nullptr), currentFunction(nullptr), currentFunctionDeclaration(nullptr),
    currentAccessSpecifier(Specifier::public_), conversionFunctionType(nullptr)
{
}
...
void TypeBinder::Visit(IncludeDirectiveNode& includeDirectiveNode)
{
    SourceFileNode* includeFile = includeDirectiveNode.GetSourceFileNode();
    if (includeFile)
    {
        if (includeFile->IncludeDirectives())
        {
            includeFile->IncludeDirectives()->Accept(*this);
        }
        for (Node* usingDirectiveOrDeclaration : includeFile->UsingDirectivesAndDeclarations())
        {
            usingDirectiveOrDeclaration->Accept(*this);
        }
        if (!includeFile->Processed())
        {
            includeDirectiveNode.GetSourceFileNode()->Accept(*this);
        }
    }
}

void TypeBinder::Visit(IncludeDirectiveSequenceNode& includeDirectiveSequenceNode)
{
...
void TypeBinder::Visit(SimpleDeclarationNode& simpleDeclarationNode)
{
    ...
    {
        TypeSymbol* type = ResolveType(symbolTable, containerScope, *currentSourceFile, simpleDeclarationNode.TypeExpr());
...
void TypeBinder::Visit(TypedefNode& typedefNode)
{
    ...
    if (symbol)
    {
        if (symbol->IsTypedefSymbol())
        {
            ...
            TypeSymbol* type = ResolveType(symbolTable, containerScope, *currentSourceFile, typedefNode.TypeExpr());
...
void TypeBinder::Visit(EnumTypeNode& enumTypeNode)
{
    ...
    if (symbol->IsEnumTypeSymbol())
    {
        ...
        {
            TypeSymbol* baseType = ResolveType(symbolTable, containerScope, *currentSourceFile, enumTypeNode.EnumBase());
...
{
    ResolveType(symbolTable, containerScope, *currentSourceFile, TypeResolverFlags::nothrow, forwardClassDeclarationNode.ClassName());
}

void TypeBinder::Visit(ElaborateClassNameNode& elaborateClassNameNode)
{
    ResolveType(symbolTable, containerScope, *currentSourceFile, TypeResolverFlags::nothrow, elaborateClassNameNode.ClassName());
}
...
{
    TypeSymbol* baseClass = ResolveType(symbolTable, containerScope, *currentSourceFile, baseClassSpecifierNode.ClassName());
...
void TypeBinder::Visit(MemberDeclarationNode& memberDeclarationNode)
{
    ...
    {
        TypeSymbol* type = ResolveType(symbolTable, containerScope, *currentSourceFile, memberDeclarationNode.TypeExpr());
...
void TypeBinder::Visit(FunctionNode& functionNode)
{
    ...
    if (currentFunction && currentFunction->FunctionGroup())
    {
        ...
        {
            TypeSymbol* returnType = ResolveType(symbolTable, containerScope, *currentSourceFile, functionNode.TypeExpr());
...
void TypeBinder::Visit(ParameterNode& parameterNode)
{
    ...
    if (symbol->IsParameterSymbol())
    {
        ...
        TypeSymbol* type = ResolveType(symbolTable, containerScope, *currentSourceFile, parameterNode.TypeExpr());
...
{
    conversionFunctionType = ResolveType(symbolTable, containerScope, *currentSourceFile, conversionFunctionIdNode.TypeExpr());
}
...
void TypeBinder::Visit(SimpleTypeCastExpressionNode& simpleTypeCastExpressionNode)
{
...
{
    TypeSymbol* type = ResolveType(symbolTable, containerScope, *currentSourceFile, TypeResolverFlags::nothrow | TypeResolverFlags::noExternalTypes, memberInitializerNode.Id());
