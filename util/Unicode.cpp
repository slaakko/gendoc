...
namespace gendoc { namespace unicode {
...
std::string MakeCanonicalPropertyName(const std::string& s)
{
    ...
    return gendoc::util::ToLower(propertyName);
}
...
const uint8_t headerMagic[8] =
{
    ...
    static_cast<uint8_t>('U'), static_cast<uint8_t>('C'), static_cast<uint8_t>('D'), current_gendoc_ucd_version
};

std::string GenDocRoot()
{
    std::string gendocRoot;
    const char* gendocRootEnv = getenv("GENDOC_ROOT");
    if (gendocRootEnv)
    {
        gendocRoot = gendocRootEnv;
    }
    if (gendocRoot.empty())
    {
        throw UnicodeException("please set 'GENDOC_ROOT' environment variable to contain /path/to/gendoc directory.");
    }
    return gendocRoot;
}

std::string GenDocUcdFilePath()
{
    return (boost::filesystem::path(GenDocRoot()) / boost::filesystem::path("unicode") / boost::filesystem::path("gendoc_ucd.bin")).generic_string();
}
...
{
    std::string ucdFilePath = GenDocUcdFilePath();
...
void CharacterTable::ReadHeader(BinaryReader& reader)
{
    ...
    for (int i = 0; i < 7; ++i)
    {
        ...
        {
            throw UnicodeException("invalid gendoc_ucd.bin header magic: 'CMAJUCD' expected");
        }
    }
    if (magic[7] != headerMagic[7])
    {
        throw UnicodeException("invalid gendoc_ucd.bin version: version " + std::string(1, headerMagic[7]) + " expected, version " + std::string(1, magic[7]) + " read");
    }
